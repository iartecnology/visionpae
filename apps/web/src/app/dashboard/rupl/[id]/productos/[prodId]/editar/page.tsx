...
export default function EditarProductoAdminPage() {
  ...
  const params = useParams<{ id: string; prodId: string }>();
  const productorId = params.id;
  ...
  useEffect(() => {
    if (!productorId || !prodId) { setError('Producto no encontrado'); setLoading(false); return; }
    api.get<any>(`/rupl/productores/${productorId}/productos/${prodId}`)
      .then((p) => {
        setForm({
          nombre: p.nombre || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.patch(`/rupl/productores/${productorId}/productos/${prodId}`, payload);
      setSuccess(true);
      setTimeout(() => router.push(`/dashboard/rupl/${productorId}`), 1200);
    } catch (err: any) {
    ...
    <div className="mx-auto max-w-lg space-y-6">
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200/80 bg-white p-4 shadow-[0_2px_12px_-6px_rgba(0,0,0,0.08)] sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Actualizar la informacion del producto</p>
        </div>
