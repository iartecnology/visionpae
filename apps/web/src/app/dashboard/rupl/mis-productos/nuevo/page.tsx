...
export default function NuevoMiProductoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/rupl/productores/mis-productos', payload);
      setSuccess(true);
    ...
    <div className="mx-auto max-w-lg space-y-6">
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200/80 bg-white p-4 shadow-[0_2px_12px_-6px_rgba(0,0,0,0.08)] sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Agregar un producto a tu oferta</p>
        </div>
        ...
        {showBaseSearch && (
          <Card className="border-slate-200/60 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.06)]">
            <CardHeader className="border-b border-slate-100 pb-3">
              <CardTitle className="text-sm font-semibold text-slate-700">Buscar en Catalogo Base</CardTitle>
            </CardHeader>
            <CardContent className="pt-4">
              <p className="mb-3 text-xs text-slate-500">Selecciona un producto del catalogo base para pre-llenar los datos, o crealo desde cero.</p>
              <Input
                value={baseQuery}
        ...
        <Card className="mt-4 border-slate-200/60 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.06)]">
          ...
          <CardContent className="space-y-3 pt-4">
            {presentaciones.length === 0 && (
              <p className="text-xs text-slate-400">Sin presentaciones adicionales. El producto se ofrecera en la unidad seleccionada.</p>
            )}
